...
chain_blocked | Auto chain blocked due to missing future configs | campaign_id, from_phase, missing_phases[], message
...
- `chain_blocked` is emitted only in `full_sequence` mode when attempting to start the first phase or auto-advance and missing configs for future phases.
- `missing_phases` list uses internal phase identifiers: `dns_validation`, `http_keyword_validation`, `analysis`.
