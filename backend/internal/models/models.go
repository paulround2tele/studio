...
const (
	...
	JobTypeEnrichment     JobTypeEnum = "enrichment"              // @enum enrichment
...
// CampaignCompletenessEnum defines the data completeness state of a campaign.
// This is a first-class type - all API contracts must use this enum, not raw strings.
// Used to indicate whether campaign stats are final or still accumulating.
type CampaignCompletenessEnum string

const (
	CampaignCompletenessPending  CampaignCompletenessEnum = "pending"  // Campaign not started, no data yet
	CampaignCompletenessPartial  CampaignCompletenessEnum = "partial"  // Campaign running, data incomplete
	CampaignCompletenessComplete CampaignCompletenessEnum = "complete" // Campaign finished, data final
	CampaignCompletenessDegraded CampaignCompletenessEnum = "degraded" // Campaign has failures, data partial
)

// ValidCompletenessStates returns all valid completeness values
func ValidCompletenessStates() []CampaignCompletenessEnum {
	return []CampaignCompletenessEnum{
		CampaignCompletenessPending,
		CampaignCompletenessPartial,
		CampaignCompletenessComplete,
		CampaignCompletenessDegraded,
	}
}

// IsValid checks if the completeness state is a valid enum value
func (c CampaignCompletenessEnum) IsValid() bool {
	for _, valid := range ValidCompletenessStates() {
		if c == valid {
			return true
		}
	}
...
// ParseCompleteness parses a string into a CampaignCompletenessEnum, returning an error if invalid
func ParseCompleteness(s string) (CampaignCompletenessEnum, error) {
	completeness := CampaignCompletenessEnum(s)
	if !completeness.IsValid() {
		return "", fmt.Errorf("invalid completeness state: %q, valid values are: %v", s, ValidCompletenessStates())
	}
...
// DomainRejectionReasonEnum defines the reason a domain was excluded from qualified leads
// This is a first-class type - all API contracts must use this enum, not raw strings.
type DomainRejectionReasonEnum string

const (
	DomainRejectionReasonQualified   DomainRejectionReasonEnum = "qualified"    // Not rejected, became a lead
	DomainRejectionReasonLowScore    DomainRejectionReasonEnum = "low_score"    // Keywords found but score below threshold
	DomainRejectionReasonNoKeywords  DomainRejectionReasonEnum = "no_keywords"  // No keyword matches found
	DomainRejectionReasonParked      DomainRejectionReasonEnum = "parked"       // Domain is parked/placeholder
	DomainRejectionReasonDNSError    DomainRejectionReasonEnum = "dns_error"    // DNS validation failed
	DomainRejectionReasonDNSTimeout  DomainRejectionReasonEnum = "dns_timeout"  // DNS validation timed out
	DomainRejectionReasonHTTPError   DomainRejectionReasonEnum = "http_error"   // HTTP validation failed
	DomainRejectionReasonHTTPTimeout DomainRejectionReasonEnum = "http_timeout" // HTTP validation timed out
	DomainRejectionReasonPending     DomainRejectionReasonEnum = "pending"      // Validation not yet complete
)

// ValidRejectionReasons returns all valid rejection reason values
func ValidRejectionReasons() []DomainRejectionReasonEnum {
	return []DomainRejectionReasonEnum{
...
// IsValid checks if the rejection reason is a valid enum value
func (r DomainRejectionReasonEnum) IsValid() bool {
	for _, valid := range ValidRejectionReasons() {
		if r == valid {
			return true
		}
	}
...
// ParseRejectionReason parses a string into a DomainRejectionReasonEnum, returning an error if invalid
func ParseRejectionReason(s string) (DomainRejectionReasonEnum, error) {
	reason := DomainRejectionReasonEnum(s)
	if !reason.IsValid() {
		return "", fmt.Errorf("invalid rejection reason: %q, valid values are: %v", s, ValidRejectionReasons())
	}
	return reason, nil
}
...
type GeneratedDomain struct {
	...
	CampaignID    uuid.UUID      `db:"campaign_id" json:"campaignId" firestore:"campaignId" validate:"required"`
	DomainName    string         `db:"domain_name" json:"domainName" firestore:"domainName" validate:"required,hostname_rfc1123"`
	OffsetIndex   int64          `db:"offset_index" json:"offsetIndex" firestore:"offsetIndex" validate:"gte=0"`
	...
	// Rejection tracking (P0-1)
	RejectionReason *DomainRejectionReasonEnum `db:"rejection_reason" json:"rejectionReason,omitempty" firestore:"rejectionReason,omitempty"`
	...
	SecondaryPagesExamined int16           `db:"secondary_pages_examined" json:"secondaryPagesExamined"`
