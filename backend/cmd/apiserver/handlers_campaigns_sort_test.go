...
func TestSortMetricValue(t *testing.T) {
    ...
    if v := sortMetricValue(item, "richness_score"); v != 12.5 { t.Fatalf("expected 12.5 got %v", v) }
    if v := sortMetricValue(item, "microcrawl_gain"); (v < 0.419 || v > 0.421) { t.Fatalf("expected ~0.42 got %v", v) }
    if v := sortMetricValue(item, "keywords_unique"); v != 7 { t.Fatalf("expected 7 got %v", v) }
    empty := gen.DomainListItem{}
    if v := sortMetricValue(empty, "richness_score"); v > -1e100 { t.Fatalf("expected negInf sentinel got %v", v) }
}
...
func TestWarningsFilterLogic(t *testing.T) {
    ...
    cases := []struct{ it gen.DomainListItem; expectWarn bool }{
        {mk(0,0,0), false},
        {mk(0.1,0,0), true},
        {mk(0,0.31,0), true},
        {mk(0,0,0.41), true},
    ...
    for i,c := range cases {
        ...
        if hasWarn != c.expectWarn { t.Fatalf("case %d expected warn=%v got %v", i, c.expectWarn, hasWarn) }
    }
}
