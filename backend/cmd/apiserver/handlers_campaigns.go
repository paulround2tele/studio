...
func (h *strictHandlers) CampaignsPhaseConfigure(ctx context.Context, r gen.CampaignsPhaseConfigureRequestObject) (gen.CampaignsPhaseConfigureResponseObject, error) {
	...
	// Persist config only after successful configuration/validation.
...
func mapAPIPhaseToModel(p string) (models.PhaseTypeEnum, error) {
	...
	case string(models.PhaseTypeDomainGeneration),
		string(models.PhaseTypeDNSValidation),
		string(models.PhaseTypeHTTPKeywordValidation),
		string(models.PhaseTypeExtraction),
		string(models.PhaseTypeEnrichment),
		string(models.PhaseTypeAnalysis):
...
func buildPhaseStatusResponse(phase models.PhaseTypeEnum, st *domainservices.PhaseStatus) gen.PhaseStatusResponse {
	...
	var runtimeControlsPayload interface{}
	if st.Configuration != nil {
		if val, ok := st.Configuration["runtime_controls"]; ok {
			runtimeControlsPayload = val
		}
		if len(st.Configuration) > 0 {
	...
	if runtimeControlsPayload != nil {
		if mapped := convertRuntimeControls(runtimeControlsPayload); mapped != nil {
			resp.RuntimeControls = mapped
		}
	} else {
		// Some phase services only include runtime control capabilities when an in-memory execution
		// exists; after restarts we can still have persisted paused/running state without an
		// attached runtime_controls payload. Provide a conservative default so the UI can
		// enable controls based on status.
		caps := domainservices.PhaseControlCapabilities{
			CanPause:   st.Status == models.PhaseStatusInProgress,
			CanResume:  st.Status == models.PhaseStatusPaused,
			CanStop:    st.Status == models.PhaseStatusInProgress || st.Status == models.PhaseStatusPaused,
			CanRestart: st.Status == models.PhaseStatusFailed || st.Status == models.PhaseStatusCompleted,
		}
		resp.RuntimeControls = runtimeControlsFromCapabilities(caps)
	}
...
func convertRuntimeControls(payload interface{}) *gen.PhaseRuntimeControls {
	caps := parsePhaseControlCapabilities(payload)
	if caps == nil {
		return nil
	}
	return runtimeControlsFromCapabilities(*caps)
}

type controlCapability string

const (
	controlCapabilityPause  controlCapability = "pause"
	controlCapabilityResume controlCapability = "resume"
	controlCapabilityStop   controlCapability = "stop"
)

func (c controlCapability) label() string {
	switch c {
	case controlCapabilityPause:
		return "pausing"
	case controlCapabilityResume:
		return "resuming"
	case controlCapabilityStop:
		return "stopping"
	default:
		return string(c)
	}
...
func parsePhaseControlCapabilities(payload interface{}) *domainservices.PhaseControlCapabilities {
	switch rc := payload.(type) {
	case domainservices.PhaseControlCapabilities:
		clone := rc
		return &clone
	case *domainservices.PhaseControlCapabilities:
		if rc == nil {
		...
		return rc
	case map[string]interface{}:
		return &domainservices.PhaseControlCapabilities{
			CanPause:   boolFromInterfaceMap(rc, "can_pause", "canPause"),
			CanResume:  boolFromInterfaceMap(rc, "can_resume", "canResume"),
			CanStop:    boolFromInterfaceMap(rc, "can_stop", "canStop"),
			CanRestart: boolFromInterfaceMap(rc, "can_restart", "canRestart"),
		}
	case gen.PhaseRuntimeControls:
		return &domainservices.PhaseControlCapabilities{
			CanPause:   rc.CanPause,
			CanResume:  rc.CanResume,
			CanStop:    rc.CanStop,
			CanRestart: rc.CanRestart,
		}
	case *gen.PhaseRuntimeControls:
		if rc == nil {
			return nil
...
func (h *strictHandlers) ensurePhaseSupportsControl(ctx context.Context, campaignID uuid.UUID, phase models.PhaseTypeEnum, capability controlCapability) error {
	caps := h.getPhaseControlCapabilities(ctx, campaignID, phase)
	if caps == nil {
		return nil
	}
	var allowed bool
	switch capability {
	case controlCapabilityPause:
		allowed = caps.CanPause
	case controlCapabilityResume:
		allowed = caps.CanResume
	case controlCapabilityStop:
		allowed = caps.CanStop
	default:
		allowed = true
	}
	if allowed {
	...
	phaseName := phases.ToAPI(string(phase))
	return fmt.Errorf("%s phase does not support %s", phaseName, capability.label())
}

func (h *strictHandlers) getPhaseControlCapabilities(ctx context.Context, campaignID uuid.UUID, phase models.PhaseTypeEnum) *domainservices.PhaseControlCapabilities {
	if h.deps == nil || h.deps.Orchestrator == nil {
		return nil
	}
	st, err := h.deps.Orchestrator.GetPhaseStatus(ctx, campaignID, phase)
	if err != nil || st == nil || st.Configuration == nil {
		return nil
	}
	payload, ok := st.Configuration["runtime_controls"]
	if !ok {
		return nil
	}
	return parsePhaseControlCapabilities(payload)
}

func runtimeControlsFromCapabilities(caps domainservices.PhaseControlCapabilities) *gen.PhaseRuntimeControls {
	return &gen.PhaseRuntimeControls{
		CanPause:   caps.CanPause,
		CanResume:  caps.CanResume,
		CanStop:    caps.CanStop,
		CanRestart: caps.CanRestart,
	}
}

func boolFromInterfaceMap(m map[string]interface{}, keys ...string) bool {
	for _, key := range keys {
		if val, ok := m[key]; ok {
			if res, found := coerceBool(val); found {
				return res
			}
		}
	}
...
func coerceBool(val interface{}) (bool, bool) {
	switch v := val.(type) {
	case bool:
		return v, true
	case *bool:
...
func (h *strictHandlers) CampaignsPhasePause(ctx context.Context, r gen.CampaignsPhasePauseRequestObject) (gen.CampaignsPhasePauseResponseObject, error) {
	...
	if err := h.deps.Orchestrator.PausePhaseWithOpts(ctx, campaignID, phaseModel, controlOpts); err != nil {
		...
		case errors.Is(err, domainservices.ErrPhasePauseUnsupported):
			...
			return gen.CampaignsPhasePause400JSONResponse{BadRequestJSONResponse: gen.BadRequestJSONResponse{Error: gen.ApiError{Message: msg, Code: gen.BADREQUEST, Timestamp: time.Now()}, RequestId: reqID(), Success: boolPtr(false)}}, nil
		case errors.Is(err, domainservices.ErrPhaseExecutionMissing), errors.Is(err, domainservices.ErrPhaseNotRunning):
			msg := fmt.Sprintf("%s phase is not currently running", phases.ToAPI(string(phaseModel)))
			return gen.CampaignsPhasePause400JSONResponse{BadRequestJSONResponse: gen.BadRequestJSONResponse{Error: gen.ApiError{Message: msg, Code: gen.BADREQUEST, Timestamp: time.Now()}, RequestId: reqID(), Success: boolPtr(false)}}, nil
		case errors.Is(err, domainservices.ErrPhasePauseTimeout):
			return gen.CampaignsPhasePause500JSONResponse{InternalServerErrorJSONResponse: gen.InternalServerErrorJSONResponse{Error: gen.ApiError{Message: "pause request timed out", Code: gen.INTERNALSERVERERROR, Timestamp: time.Now()}, RequestId: reqID(), Success: boolPtr(false)}}, nil
		default:
	...
	// The campaign status endpoint is cached (TTL). Invalidate so subsequent snapshot reads reflect
	// the pause immediately and don't briefly revert due to stale cache.
	if h.deps.AggregatesCache != nil {
		h.deps.AggregatesCache.InvalidateStatus(campaignID)
	}
	st, _ := h.deps.Orchestrator.GetPhaseStatus(ctx, campaignID, phaseModel)
	data := buildPhaseStatusResponse(phaseModel, st)
...
// CampaignsPhaseResume implements POST /campaigns/{campaignId}/phase/{phase}/resume
func (h *strictHandlers) CampaignsPhaseResume(ctx context.Context, r gen.CampaignsPhaseResumeRequestObject) (gen.CampaignsPhaseResumeResponseObject, error) {
	if h.deps == nil || h.deps.Orchestrator == nil || h.deps.Stores.Campaign == nil || h.deps.DB == nil {
		return gen.CampaignsPhaseResume401JSONResponse{UnauthorizedJSONResponse: gen.UnauthorizedJSONResponse{Error: gen.ApiError{Message: "dependencies not ready", Code: gen.UNAUTHORIZED, Timestamp: time.Now()}, RequestId: reqID(), Success: boolPtr(false)}}, nil
	}
	campaignID := uuid.UUID(r.CampaignId)
	if _, err := h.deps.Stores.Campaign.GetCampaignByID(ctx, h.deps.DB, campaignID); err != nil {
		if err == store.ErrNotFound {
			return gen.CampaignsPhaseResume404JSONResponse{NotFoundJSONResponse: gen.NotFoundJSONResponse{Error: gen.ApiError{Message: "campaign not found", Code: gen.NOTFOUND, Timestamp: time.Now()}, RequestId: reqID(), Success: boolPtr(false)}}, nil
		}
		return gen.CampaignsPhaseResume500JSONResponse{InternalServerErrorJSONResponse: gen.InternalServerErrorJSONResponse{Error: gen.ApiError{Message: "failed to fetch campaign", Code: gen.INTERNALSERVERERROR, Timestamp: time.Now()}, RequestId: reqID(), Success: boolPtr(false)}}, nil
	}
	phaseModel, err := mapAPIPhaseToModel(string(r.Phase))
	if err != nil {
		return gen.CampaignsPhaseResume400JSONResponse{BadRequestJSONResponse: gen.BadRequestJSONResponse{Error: gen.ApiError{Message: "invalid phase", Code: gen.BADREQUEST, Timestamp: time.Now()}, RequestId: reqID(), Success: boolPtr(false)}}, nil
	}
	...
	if err := h.deps.Orchestrator.ResumePhaseWithOpts(ctx, campaignID, phaseModel, controlOpts); err != nil {
		...
		case errors.Is(err, domainservices.ErrPhaseResumeUnsupported):
			...
			return gen.CampaignsPhaseResume400JSONResponse{BadRequestJSONResponse: gen.BadRequestJSONResponse{Error: gen.ApiError{Message: msg, Code: gen.BADREQUEST, Timestamp: time.Now()}, RequestId: reqID(), Success: boolPtr(false)}}, nil
		case errors.Is(err, domainservices.ErrPhaseExecutionMissing), errors.Is(err, domainservices.ErrPhaseNotPaused), errors.Is(err, domainservices.ErrPhaseNotRunning):
			msg := fmt.Sprintf("%s phase is not currently paused", phases.ToAPI(string(phaseModel)))
			return gen.CampaignsPhaseResume400JSONResponse{BadRequestJSONResponse: gen.BadRequestJSONResponse{Error: gen.ApiError{Message: msg, Code: gen.BADREQUEST, Timestamp: time.Now()}, RequestId: reqID(), Success: boolPtr(false)}}, nil
		case errors.Is(err, application.ErrAnotherPhaseRunning):
			msg := "another phase is already running; stop or pause it before resuming a different phase"
			return gen.CampaignsPhaseResume409JSONResponse{ConflictJSONResponse: gen.ConflictJSONResponse{Error: gen.ApiError{Message: msg, Code: gen.CONFLICT, Timestamp: time.Now()}, RequestId: reqID(), Success: boolPtr(false)}}, nil
		default:
			return gen.CampaignsPhaseResume500JSONResponse{InternalServerErrorJSONResponse: gen.InternalServerErrorJSONResponse{Error: gen.ApiError{Message: "failed to resume phase: " + err.Error(), Code: gen.INTERNALSERVERERROR, Timestamp: time.Now()}, RequestId: reqID(), Success: boolPtr(false)}}, nil
		}
	}
	// The campaign status endpoint is cached (TTL). Invalidate so subsequent snapshot reads reflect
	...
	st, _ := h.deps.Orchestrator.GetPhaseStatus(ctx, campaignID, phaseModel)
	data := buildPhaseStatusResponse(phaseModel, st)
	return gen.CampaignsPhaseResume200JSONResponse(data), nil
}
