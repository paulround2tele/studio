-- BL-007 Schema fixes for existing tables
BEGIN;

-- Fix suspicious_input_patterns table schema
ALTER TABLE suspicious_input_patterns 
    ADD COLUMN IF NOT EXISTS category VARCHAR(50) NOT NULL DEFAULT 'security',
    ADD COLUMN IF NOT EXISTS is_enabled BOOLEAN DEFAULT true;

-- Rename columns to match code expectations
ALTER TABLE suspicious_input_patterns 
    RENAME COLUMN pattern_regex TO pattern;

ALTER TABLE suspicious_input_patterns 
    RENAME COLUMN risk_level TO severity;

-- Create suspicious_input_alerts table if it doesn't exist
...
-- Insert suspicious patterns if they don't exist
