-- Migration: 013_si005_memory_monitoring.sql
-- Purpose: SI-005 Memory Management Issues - Database schema for memory monitoring and optimization
-- Author: SI-005 Implementation Team
-- Date: 2025-06-22
...
BEGIN
    IF EXISTS (SELECT 1 FROM public.schema_migrations WHERE version = 13) THEN
        RAISE EXCEPTION 'Migration 13 already applied';
    END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_memory_metrics_service_recorded ON memory_metrics(service_name, recorded_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_memory_allocations_leaked ON memory_allocations(memory_leaked_bytes) WHERE memory_leaked_bytes > 0;
...
CREATE INDEX IF NOT EXISTS idx_memory_leak_detected ON memory_leak_detection(detected_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_memory_optimization_priority ON memory_optimization_recommendations(implementation_priority);
CREATE INDEX IF NOT EXISTS idx_memory_optimization_implemented ON memory_optimization_recommendations(implemented);

-- System alerts table already exists, just ensure indexes exist
