-- Rollback trigger function enum fix
-- Restore the previous version that used 'running' (broken)
...
BEGIN
    ...
    IF OLD.current_phase IS DISTINCT FROM NEW.current_phase THEN
        ...
        -- ❌ BROKEN: Start new phase with wrong enum value (for rollback only)
        IF NEW.current_phase IS NOT NULL THEN
            UPDATE campaign_phases
            SET
                status = 'running',              -- ❌ BROKEN: 'running' is not valid for phase_status_enum
