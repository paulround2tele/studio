...
export const campaignStateSyncMiddleware: Middleware = (store) => (next) => (action) => {
  ...
  const allowed: ReadonlyArray<string> = ['discovery','validation','extraction','analysis','enrichment'];
  const phase = (allowed.includes(rawPhase) ? rawPhase : normalizeToApiPhase(rawPhase) || 'discovery') as 'discovery' | 'validation' | 'extraction' | 'analysis' | 'enrichment';
