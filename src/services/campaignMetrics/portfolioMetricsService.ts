...
function calculateTrend(snapshots: AggregateSnapshot[]): 'up' | 'down' | 'stable' {
  // Need at least two valid snapshots
  if (!Array.isArray(snapshots) || snapshots.length < 2) {
    return 'stable';
  }

  // Defensive filter: ensure all have timestamp & aggregates
  const valid = snapshots.filter(s => s && s.timestamp && s.aggregates);
  ...
  const latest = sorted.at(-1)!; // safe due to length check
  const previous = sorted.at(-2)!;

  const latestScore = (latest.aggregates.successRate ?? 0) as number;
  const previousScore = (previous.aggregates.successRate ?? 0) as number;
