...
import { isBackendCanonical as backendCanonicalEnabled } from '@/lib/feature-flags-simple';
...
class EnhancedRecommendationService {
  ...
  /**
   * Generate layered recommendations from multiple sources
   */
  async generateLayeredRecommendations(
    ...
    const anomalies = recommendationContext?.anomalies;
    if (config.enableAnomalyAugmentation && anomalies) {
    ...
    const heuristicRecs = this.generateClientHeuristicRecommendations(
      snapshots,
      (recommendationContext ?? {}) as Record<string, unknown>
    );
  ...
  private async getExperimentalMLRecommendations(
    ...
    try {
      ...
      interface RawExp { [k: string]: unknown; confidence?: unknown; reasoning?: unknown; baselineComparison?: unknown }
      return experimentalRecs.slice(0, config.maxExperimentalRecs).filter(r => r && typeof r === 'object').map(r => {
        const rec = r as unknown as RawExp;
        return {
          ...rec,
          provenance: {
            ...
            reasoning: Array.isArray(rec.reasoning) ? rec.reasoning as string[] : ['Generated by experimental ML model'],
            baselineComparison: rec.baselineComparison as unknown
          }
        } as EnhancedRecommendation;
      });
    } catch (error) {
  ...
  const aggregatesRec = (latest && typeof latest === 'object') ? (latest as unknown as { aggregates?: Record<string, unknown> }).aggregates : undefined;
    const aggregates = (aggregatesRec && typeof aggregatesRec === 'object') ? aggregatesRec as Record<string, unknown> : {};
    const successRate = typeof aggregates['successRate'] === 'number' ? aggregates['successRate'] as number : Number.NaN;
    const avgLeadScore = typeof aggregates['avgLeadScore'] === 'number' ? aggregates['avgLeadScore'] as number : Number.NaN;

    // Low success rate recommendation
  if (typeof successRate === 'number' && !Number.isNaN(successRate) && successRate < 0.7) {
      recommendations.push({
        id: `heuristic_success_rate_${Date.now()}`,
        severity: 'warn',
        title: 'Improve Campaign Success Rate',
  detail: `Current success rate is ${(successRate * 100).toFixed(1)}%, below optimal threshold`,
  ...
  if (typeof avgLeadScore === 'number' && !Number.isNaN(avgLeadScore) && avgLeadScore < 30) {
      recommendations.push({
        id: `heuristic_lead_score_${Date.now()}`,
        severity: 'action',
        title: 'Optimize Lead Scoring Model',
  detail: `Average lead score is ${avgLeadScore.toFixed(1)}, indicating low-quality domains`,
  ...
  private categorizeRecommendation(rec: Record<string, unknown>): EnhancedRecommendation['category'] {
    ...
    const title = typeof titleVal === 'string' ? titleVal.toLowerCase() : '';
  ...
  private calculatePriority(rec: Record<string, unknown>): number {
    ...
    const severityVal = rec['severity'];
    const basePriority = severityVal === 'action' ? 8 : severityVal === 'warn' ? 6 : 4;
    const confidenceVal = rec['confidence'];
    const confidence = typeof confidenceVal === 'number' ? confidenceVal : 0;
  ...
  private makeActionable(rec: Record<string, unknown>): EnhancedRecommendation['actionable'] {
    return {
      quickAction: typeof rec['detail'] === 'string' ? rec['detail'] as string : undefined,
      automatable: false
    };
  }
  ...
  public getDefaultConfig(): ExperimentalRecommendationConfig {
    return {
...
export async function generateLayeredRecommendations(
  ...
  const mergedConfig: ExperimentalRecommendationConfig = {
    ...enhancedRecommendationService.getDefaultConfig(),
    ...config,
  };
