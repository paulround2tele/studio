...
const mockUsePaginatedDomains = jest.fn();
...
jest.mock('@/lib/hooks/usePaginatedDomains', () => ({
  usePaginatedDomains: (...args: unknown[]) => mockUsePaginatedDomains(...args),
}));

jest.mock('@/hooks/useCampaignSSE', () => ({
  useCampaignSSE: jest.fn(),
}));

jest.mock('@/store/api/scoringApi', () => ({
  useListScoringProfilesQuery: jest.fn().mockReturnValue({ data: { items: [] } }),
}));
...
const createPaginatedMock = (items = sampleItems) => {
  ...
  return [paginatedState, apiControls] as const;
};

beforeEach(() => {
  jest.clearAllMocks();
  mockUseGetCampaignEnrichedQuery.mockReturnValue({
    data: { campaign: { id: 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa' } },
    isLoading: false,
    error: undefined,
  });
  mockUsePaginatedDomains.mockReturnValue(createPaginatedMock());
});
...
describe('DomainsList features integration', () => {
  it('renders richness badge and top keywords from features', async () => {
    ...
    const badge = within(firstRow).getByTestId('richness-badge');
    expect(badge.textContent).toMatch(/0\.82/);
    const kwCell = within(firstRow).getByTestId('campaign-domains-cell-topkeywords');
